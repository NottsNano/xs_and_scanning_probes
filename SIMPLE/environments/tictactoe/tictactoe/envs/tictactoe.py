...
class TicTacToeEnv(gym.Env):
    ...
    def step(self, action):
        # print(self.current_player.token.symbol)
        reward = [0, 0]

        # check move legality
        board = self.board

        if (board[action].number != 0):  # not empty - move is illegal
            done = True
            reward = [0, 0] #[1,1]
            reward[self.current_player_num] = -10 #-1
    ...
    def render(self, mode='human', close=False, verbose=True):
        ...
        if self.render_mode is None:
            ...
            reshaped_board = board.reshape((3, 3)).T

            x_to_plot = np.argwhere(reshaped_board == 1)
            o_to_plot = np.argwhere(reshaped_board == -1)
            if len(x_to_plot):
                self.xplot.set_data(x_to_plot[:, 0], x_to_plot[:, 1])
                self.xplot.set_linestyle('None')
            if len(o_to_plot):
                self.oplot.set_data(o_to_plot[:, 0], o_to_plot[:, 1])
                self.oplot.set_linestyle('None')
            plt.pause(0.001)
        else:
